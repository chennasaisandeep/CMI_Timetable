...
class TimetableProcessor:
    ...
    def extract_timetable_data(self, timetable):
        ...
        return (
            "\n".join(lines[:optional_course_index])
            if optional_course_index
            else timetable
        )
    ...
    def _is_valid_index(self, row_idx, col_idx):
        ...
        return (
            0 <= row_idx < self.num_rows and 1 <= col_idx < self.num_cols
        )  # Col 0 reserved for days
    ...
    def add_extra_row(self, row_name):
        ...
        new_row = pd.DataFrame(
            [[row_name] + [""] * (len(self.df.columns) - 1)], columns=self.df.columns
        )
...
class SubjectTimetableManager:
    ...
    def initialize_mappings(self):
        ...
        course_divs = self.page_content_div.find_all(
            "div", id=lambda x: x and x.startswith("prog_")
        )
        ...
        for div in course_divs:
            ...
            try:
                ...
                if self.timings is None:
                    self.timings = processor.df.columns[
                        1:
                    ].tolist()  # Exclude 'Day' column
                    ...
                    for j in range(1, processor.num_cols):  # Skip day column
                        ...
                        if pd.notna(subject) and subject.strip():
                            ...
                            course_subjects[subject].append(
                                {"day": day, "timing": timing, "row": i, "col": j}
                            )
...
def render_timetable(
    ...
    if (
        sat_row.iloc[:, 1:].apply(lambda x: x == "").all().all()
    ):  # Check if all cells except 'Day' are empty
...
# Function to create iCalendar entries
def create_ics(df):
    ics = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//CMI_student//Streamlit_timetable//EN",
    ]
    days_map = {
        "MON": "MO",
        "TUE": "TU",
    ...
    for _, row in df.iterrows():
        day = days_map[row["Day"]]  # Map day to iCal abbreviation
        for time_slot, event in row.items():
            if time_slot != "Day" and event:  # Skip the "Day" column and empty events
                # Calculate the next occurrence of the day
                today = datetime.now()
                target_day = list(days_map.keys()).index(
                    row["Day"]
                )  # Index of target day (0 = MON)
                current_day = today.weekday()  # Index of current day (0 = MON)
                days_until_target = (
                    target_day - current_day + 7
                ) % 7 or 7  # Days to next target day
                event_date = today + timedelta(days=days_until_target)

                # Split and format the time range
                start_time, end_time = time_slot.split("-")
                start_time = start_time.replace(":", "")
                end_time = end_time.replace(":", "")

                # Format the start and end datetime strings in IST (Asia/Kolkata timezone)
                event_start = event_date.strftime(f"%Y%m%dT{start_time}00")
                event_end = event_date.strftime(f"%Y%m%dT{end_time}00")

                # Append the VEVENT to the calendar
                ics.append("BEGIN:VEVENT")
                ics.append(f"UID:{row['Day']}-{start_time}@example.com")
                ics.append("DTSTAMP:20250115T123456Z")  # Timestamp for creation
                ics.append(
                    f"DTSTART;TZID=Asia/Kolkata:{event_start}"
                )  # Start time in IST
                ics.append(f"DTEND;TZID=Asia/Kolkata:{event_end}")  # End time in IST
                ics.append(f"RRULE:FREQ=WEEKLY;BYDAY={day}")  # Weekly recurrence
                ics.append(f"SUMMARY:{event}")  # Event name
...
def main():
    ...
    if manager:
        ...
        with col1:
            if st.button(
                "Generate Preview", key="preview_button", use_container_width=True
            ):
                on_generate_preview()
        ...
        if st.session_state.show_preview:
            if st.session_state.current_df is None:
                st.session_state.current_df = create_initial_dataframe(
                    manager, st.session_state.selected_subjects
                )
        ...
        if st.session_state.show_final and st.session_state.current_df is not None:
            ...
            ical_data = create_ics(st.session_state.current_df)

            # lets ical_data in pastebin and get the link
            pastebin_url = "https://pastebin.com/"

            # Access the API key
            api_key = st.secrets["pastebin_api_key"]
            response = requests.post(
                pastebin_url + "api/api_post.php",
                data={
                    "api_dev_key": api_key,
                    "api_option": "paste",
                    "api_paste_code": ical_data,
                },
            )
            pastebin_link = "https://pastebin.com/raw/" + response.text.split("/")[-1]

            # show them the ical link on the page, in a copyable format, if they click on the block,
            # the link will be copied to their clipboard, and they can paste it in their calendar app
            # also they can see the whole link and copy it if they want
            ical_link = f"""
                <div style="background-color: #262730; padding: 10px; border-radius: 5px; margin-top: 10px;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div style="font-size: 16px; font-weight: bold;">iCal Link</div>
                        <div style="font-size: 14px; color: white; cursor: pointer;" onclick="navigator.clipboard.writeText('{pastebin_link}'); alert('Copied to clipboard!');">Copy</div>
                    </div>
                    <div style="font-size: 14px; margin-top: 5px;">{pastebin_link}</div>
                </div>
            """

            st.markdown(ical_link, unsafe_allow_html=True)
            plt.close(fig)
